...
package com.google.ai.edge.examples.d_fine.view
...
import com.google.ai.edge.examples.d_fine.DFine
import com.google.ai.edge.examples.d_fine.DetectionBox
 ...
 * Box coordinates are normalized in the squared [DFine.SIZE] space, so they scale with the image.
 */
@Composable
  ...
  Canvas(modifier) {
    ...
    drawIntoCanvas { canvas ->
      ...
      for (box in boxes) {
        ...
        val left = (box.cx - box.width / 2) * DFine.SIZE * scale
        val top = (box.cy - box.height / 2) * DFine.SIZE * scale
        val right = (box.cx + box.width / 2) * DFine.SIZE * scale
        val bottom = (box.cy + box.height / 2) * DFine.SIZE * scale
