...
`DFine`. The view model confines every model call to one worker, since `DFine` reuses native buffers.
...
| `android/.../d_fine/MainActivity.kt` | Thin Compose host: wires the gallery picker to the view model |
| `android/.../d_fine/MainViewModel.kt` | Owns `DFine`, runs detection off the main thread, exposes `UiState` |
| `android/.../d_fine/UiState.kt` | Immutable UI state + the display-ready `DetectionBox` |
| `android/.../d_fine/ImageUtils.kt` | Asset/gallery decode, EXIF orientation, square resize, RGB conversion |
| `android/.../d_fine/view/DetectionScreen.kt` | Compose UI: status header, image picker, results list |
| `android/.../d_fine/view/DetectionOverlay.kt` | Draws the image and detection boxes on a Compose `Canvas` |
| `android/.../d_fine/view/Theme.kt`, `view/Color.kt` | App theme and the per-class box palette |
