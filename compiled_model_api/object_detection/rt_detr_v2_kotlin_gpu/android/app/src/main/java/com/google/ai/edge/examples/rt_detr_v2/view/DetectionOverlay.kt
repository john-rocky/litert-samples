...
import com.google.ai.edge.examples.rt_detr_v2.RtDetr
 ...
 * Box coordinates are normalized in the squared [RtDetr.SIZE] space, so they scale with the image.
 */
@Composable
  ...
  Canvas(modifier) {
    ...
    drawIntoCanvas { canvas ->
      ...
      for (box in boxes) {
        ...
        val left = (box.cx - box.width / 2) * RtDetr.SIZE * scale
        val top = (box.cy - box.height / 2) * RtDetr.SIZE * scale
        val right = (box.cx + box.width / 2) * RtDetr.SIZE * scale
        val bottom = (box.cy + box.height / 2) * RtDetr.SIZE * scale
