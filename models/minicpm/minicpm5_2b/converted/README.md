...
- **Metadata**: `--check-metadata` passes on both published files (template byte-equal to the repo's `chat_template.jinja`, start token `<s>`, stop ids {1, 130073}, thought channel `<think>`/`</think>`, max_num_tokens 4096, no zero blockwise scales, int8 declares fp32 activations).
