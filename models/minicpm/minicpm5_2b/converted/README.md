...
- **Rebuilt from this directory (2026-09-09, the pinned stack above, source checkpoint sha256 `14fb8e7f…` verified)**: `build_minicpm5_2b.py --recipe int4` and `--recipe int8` (7.5 min for both in parallel on an M4 Max) reproduce the published bundles **byte-identical in every weight and tokenizer section** — `HF_Tokenizer_Zlib`, `tf_lite_prefill_decode` (1,281,048,576 B int4 / 2,330,645,824 B int8) and `tf_lite_embedder` (270,524,336 B) all hash equal; the raw int4 export differs from the published file in exactly the 3,328 bytes the zero-scale fix patches (1,664 scales in each of two tensors). The only difference is the order of the 16 `stop_tokens` entries inside `LlmMetadata` (the two ids and the 14 converter strings — same members, every other field identical; a set-iteration order in the exporter) plus the file header's uuid and timestamp. `--check-metadata` 9/9 on both rebuilt files.
